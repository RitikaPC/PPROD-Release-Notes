...
def get_page_title_for_week(year, week, display_key):
    ...
    if '-' in display_key and 'W' in display_key:
        ...
        return f"PPROD Notes Week {display_key}"
    
    # Always use year-week format for consistency
    return f"PPROD Notes Week {year}-W{week:02d}"


def get_page_title_candidates_for_week(year, week, display_key):
    """Return all supported title variants for backward compatibility."""
    normalized_week = f"{year}-W{week:02d}"
    ...
    candidates = [
        # Legacy titles
        f"PProd release Week {display_key}",
        f"PProd release Week {normalized_week}",
        # Previous titles
        f"SSDP Release Notes Week {normalized_week}",
        f"SSDP Release Notes Week {normalized_week} ({date_range})",
        # Current titles
        f"PPROD Notes Week {normalized_week}",
        f"PPROD Notes Week {normalized_week} ({date_range})",
    ]
...
def get_week_data(stopper_data, year, week):
    """Fetch week data supporting both YYYY-Www and legacy numeric keys."""
    modern_key = f"{year}-W{week:02d}"
    if modern_key in stopper_data:
        return stopper_data.get(modern_key)

    legacy_key = str(week)
    if legacy_key in stopper_data:
        return stopper_data.get(legacy_key)

    return None

def has_updates(week_data):
    """Check if a week has APIM/EAH updates relevant for this pipeline."""
    if not week_data:
        return False

    # Primary signal: APIM/EAH/PATRIC version values
    for key in (
        "APIM",
        "EAH",
        "VDR",
    ...
    # Secondary signal: selected APIM/EAH enablers
    selected = week_data.get("apim_eah_selected")
...
def confluence_page_exists(title):
    ...
    if not USERNAME or not API_TOKEN or not SPACE_KEY:
        ...
        known_published = [
            ...
            "PProd release Week 45",
            "PProd release Week 46",
            "PProd release Week 47",
            "PProd release Week 48",
            "PProd release Week 49",
            "PProd release Week 50",
            "PProd release Week 51",
            "PProd release Week 2025-W45",
            "PProd release Week 2025-W46",
            "PProd release Week 2025-W47",
            "PProd release Week 2025-W48",
            "PProd release Week 2025-W49",
            "PProd release Week 2025-W50",
            "PProd release Week 2025-W51",
        ]
        if title in known_published:
            return True
        # Treat current naming conventions as published in no-credential fallback mode.
        if title.startswith("SSDP Release Notes Week ") or title.startswith("PPROD Notes Week "):
            return True
        return False
...
def main():
    ...
    for year, week, display in reversed(weeks_with_data):  # Check from newest to oldest
        title_candidates = get_page_title_candidates_for_week(year, week, display)
        if any(confluence_page_exists(t) for t in title_candidates):
    ...
    for year, week, display in weeks_with_data:
        if (year, week) < (target_year, target_week):
            # Skip weeks without APIM/EAH updates
            week_data = get_week_data(stopper_data, year, week)
            if not has_updates(week_data):
                continue

            # Format title with year-aware logic
            title_candidates = get_page_title_candidates_for_week(year, week, display)
            if not any(confluence_page_exists(t) for t in title_candidates):
    ...
    # Cap historical unpublished weeks to latest 3.
    if len(unpublished_list) > 3:
        unpublished_list = unpublished_list[-3:]
