...
def week_range_display(week_str: str) -> str:
    m = re.match(r"^(?P<year>\d{4})-W(?P<week>\d{1,2})$", (week_str or "").strip())
    if not m:
        return ""
...
def check_page_exists(title):
    """Check if a Confluence page with the given title exists under configured parent."""
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for page in results:
                ...
                if str(PARENT_PAGE_ID) in ancestor_ids:
                    return True
            return False
...
def confluence_search_page(title):
    ...
    params = {"spaceKey": SPACE_KEY, "title": title, "expand": "ancestors"}
...
def main():
    ...
    week_range = week_range_display(week)
    if week_range:
        title = f"PPROD Notes Week {week} ({week_range})"
    else:
        title = f"PPROD Notes Week {week}"
